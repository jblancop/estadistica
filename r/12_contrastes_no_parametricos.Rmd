...
Se parte el vector muestral en dos subvectores, uno que contenga los valores superiores a la mediana y otro, los inferiores, y se ordenan de manera inversa para posteriormente poder calcular las diferencias entre los pares de valores que equidistan entre sí:
...
Todas estas variables se pueden presentar de forma resumida en una tabla, construida como *data frame*, para facilitar su visualización:
...
colnames(tabla_resumen) = c("$x_{n-i+1}$", "$x_{i}$", "$x_{n-i+1} - x_{i}$", 
...
rangos = sapply(unicos, function(x) mean(which(absolutas == x)))
rangos
```
...
```{r}
estadistico = function(diferencias_negativas_o_positivas, rangos_promedio)
{
  lista = c()
  
  for(diferencia in diferencias_negativas_o_positivas)
  {
    for(rango in rangos_promedio)
    {
      if(abs(diferencia) == which(rangos_promedio == rango))
      {
        lista = c(lista, rango)
      } 
    }
