...
En primer lugar, se construye con código la tabla bidimensional del enunciado; para ello, se combinan dos *vectores* $X$ e $Y$ definidos a su vez como agrupación de *vectores constantes* con las **frecuencias absolutas** $n_{ij}$ de cada **par de valores** $(x_{i}, y_{j})$:
...
```{r}
fi_1 = prop.table(tabla_2d, margin = 2)[ ,1]

fi_1_red = round(fi_1, 2)
fi_1_red
```
...
```{r}
fi_j = round(prop.table(tabla_2d, margin = 2), 2)
fi_j
```
...
```{r}
fj_i = round(prop.table(tabla_2d, margin = 1), 2)
fj_i
```
...
int_med = intervalos[1] #La mediana se encuentra en el inmediatamente superior
int_med
```
...
```{r}
mediana = l_med + a * ((n/2 - N.j_ant)/n.j_med)
...
int_mod = names(which(f.j == max(f.j)))
int_mod
```
...
```{r}
varianza = sum(xi^2 * fi_1) - media^2

desv_tip = sqrt(varianza)

desv_tip_red = round(desv_tip, 2)
desv_tip_red
```
...
var_x = sum(xi^2 * fi.) - media_x^2

var_x_red = round(var_x, 2)
var_x_red
```
...
var_y = sum(yj^2 * f.j) - media_y^2

var_y_red = round(var_y, 2)
var_y_red
```
...
```{r}
a = (yj * rep(xi[1], 4) * tabla_2d[1, ])/n  
b = (yj * rep(xi[2], 4) * tabla_2d[2, ])/n 
c = (yj * rep(xi[3], 4) * tabla_2d[3, ])/n
a
covarianza = sum(a, b, c) - (media_x * media_y)

covarianza_red = round(covarianza, 2)
covarianza_red
```
...
```{r}
r = round(covarianza/sqrt(var_x * var_y), 2)
r
```